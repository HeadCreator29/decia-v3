...
| Question | Resolution |
|----------|------------|
| Empty pattern lists for fallbacks? | Yes, implemented as plugins with empty patterns; classify() logic handles them |
| Explicit priority per intent? | Yes, all 16 have explicit priorities |
| pkgutil.iter_modules vs entry points? | pkgutil.iter_modules (simpler, no setuptools) |
| _has_exit_negation() location? | Moved to EXIT.validate() + kept in IntentLayer for backward compat |
| PLANNER_CREATE.validate() | Implemented with `_PLANNER_RECALL_BANNED` regex |

## Priority System (Resolved)
- GREETING=10, EXIT=10, TIME=10, DATE=10
- All others: 5
- AMBIGUOUS_INPUT=0, FREE_TALK=0
- Deterministic tie-breaking achieved
...
3. **PR #3**: 7 identity/user plugins (batched)
4. **PR #4**: 7 utility/archive plugins (batched)
5. **PR #5**: Special logic (PLANNER_CREATE.validate, EXIT.validate)
6. **PR #6**: Cleanup + verification (auto-discovery, benchmarks, docstrings)
...
- `sdd/intent-layer-plugin/apply-complete` - Phases 4-5 completion
- `sdd/intent-layer-plugin/archive-report` - This archive
